...
async def evaluate_single_task(
    ...
    response, tool_metrics = await agent_loop(client, model,
                                              qa_pair['question'], tools,
                                              connection)
    ...
    return {
        ...
        tool_metrics,
        'num_tool_calls':
        sum(len(metrics['durations']) for metrics in tool_metrics.values()),
        'summary':
        summary,
        'feedback':
        feedback,
...
async def run_evaluation(
    ...
    for i, qa_pair in enumerate(qa_pairs):
        ...
        result = await evaluate_single_task(client, model, qa_pair, tools,
                                            connection, i)
...
def parse_env_vars(env_list: list[str]) -> dict[str, str]:
    ...
    for env_var in env_list:
        if '=' in env_var:
            ...
            print(
                f'Warning: Ignoring malformed environment variable: {env_var}')
    return env
