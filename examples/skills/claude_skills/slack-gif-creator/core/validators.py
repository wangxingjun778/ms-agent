...
def validate_gif(gif_path: str | Path,
                 is_emoji: bool = True,
    ...
    if is_emoji:
        ...
        aspect_ratio = (
            max(width, height)
            / min(width, height) if min(width, height) > 0 else float('inf'))
    ...
    if verbose:
        ...
        print(f'  Dimensions: {width}x{height}'
              + (f" ({'optimal' if optimal else 'acceptable'})"
                 if is_emoji and acceptable else ''))
...
def is_slack_ready(gif_path: str | Path,
                   is_emoji: bool = True,
