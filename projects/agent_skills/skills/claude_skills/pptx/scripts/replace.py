...
def detect_frame_overflow(
        inventory: InventoryData) -> Dict[str, Dict[str, float]]:
    ...
    for slide_key, shapes_dict in inventory.items():
        for shape_key, shape_data in shapes_dict.items():
            ...
            if shape_data.frame_overflow_bottom is not None:
                ...
                overflow_map[slide_key][
                    shape_key] = shape_data.frame_overflow_bottom

    return overflow_map


def validate_replacements(inventory: InventoryData,
                          replacements: Dict) -> List[str]:
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    for slide_key, shapes_dict in inventory.items():
        ...
        for shape_key, shape_data in shapes_dict.items():
            ...
            replacement_shape_data = replacements.get(slide_key,
                                                      {}).get(shape_key, {})
