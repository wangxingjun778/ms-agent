                ...
                for elem in root.iter(f'{{{self.WORD_2006_NAMESPACE}}}t'):
                    if elem.text:
                        ...
                        if re.match(r'^\s.*', text) or re.match(
                            ...
                            if (xml_space_attr not in elem.attrib or
                                    elem.attrib[xml_space_attr] != 'preserve'):
                                # Show a preview of the text
                                text_preview = (
                                    repr(text)[:50] + '...'
                                    if len(repr(text)) > 50 else repr(text))
                                errors.append(
            ...
            try:
                ...
                for t_elem in problematic_t_elements:
                    if t_elem.text:
                        # Show a preview of the text
                        text_preview = (
                            repr(t_elem.text)[:50] + '...' if
                            len(repr(t_elem.text)) > 50 else repr(t_elem.text))
                        errors.append(
                            f'  {xml_file.relative_to(self.unpacked_dir)}: '
            ...
            try:
                ...
                for elem in invalid_elements:
                    text_preview = (
                        ...
                        + '...' if len(repr(elem.text or '')) > 50 else repr(
                            elem.text or ''))
                    errors.append(
                        f'  {xml_file.relative_to(self.unpacked_dir)}: '
